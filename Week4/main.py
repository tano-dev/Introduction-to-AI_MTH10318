                    ...
                    if start in self.get_polygon_points(polygon):
                        for poly_point in self.get_polygon_points(polygon):
                            if poly_point not in self.get_adjacent_points(start):
                                ...
                                # Them 2 dong de can_see chay dung
...
# Ham Uniform cost search
def UCS(graph, i): return i.g
...
def main():
    ...
    with open('input.txt', 'r') as f:
        ...
        a = search(graph, start, goal, a_star)  # Chay thuat toan A*
        # a = search(graph, start, goal, greedy)  # Chay thuat toan greedy
        # # Chay thuat toan uniform cost search
        # a = search(graph, start, goal, UCS)
        # a = DFS(graph, start, goal)  # Chay thuat toan depth first search
        # a = BFS(graph, start, goal)  # Chay thuat toan breadth first search
        result = list()
        while a:
            result.append(a)
            a = a.pre
